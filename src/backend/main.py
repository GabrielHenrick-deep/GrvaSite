...
@app.post("/user/")
async def register_user(user: schemaUser, session: Session = Depends(get_session)):
    db_user = ModelMembers(username=user.username, email=user.email, password=user.password)
